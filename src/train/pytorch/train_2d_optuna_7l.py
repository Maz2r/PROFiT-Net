...
def main():
    ...
    history_file = os.path.join(history_dir, f'optuna_2DCNN_{args.target_abbreviation}_7layers_independent_channels.csv')

    # Optuna 스터디 생성
    study = optuna.create_study(direction='minimize', study_name=f"optuna_study_{args.target_abbreviation}_7layers_independent_channels")
    ...
    def objective(trial):
        ...
        for repeat_idx in range(args.repeats):
            ...
            c4 = trial.suggest_categorical('c4', [32, 64, 128, 256, 512])
            c5 = trial.suggest_categorical('c5', [16, 32, 64, 128, 256])
            c6 = trial.suggest_categorical('c6', [8, 16, 32, 64, 128])
            c7 = trial.suggest_categorical('c7', [8, 16, 32, 64])

            output_channels_list = [c1, c2, c3, c4, c5, c6, c7]
    ...
    # 스터디 결과 저장
    study.trials_dataframe().to_csv(os.path.join(history_dir, f'study_2DCNN_{args.target_abbreviation}_7layers_independent_channels.csv'), index=False)
