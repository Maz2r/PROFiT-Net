...
def train_model(target_abbreviation, config):
    ...
    # 모델 생성 시 config에서 conv_params_list를 사용하도록 수정
...
if __name__ == "__main__":
    ...
    history_file = os.path.join(history_dir, f'2DCNN_{args.target_abbreviation}.csv')

    columns = ["num.", "train_loss", "train_mae", "val_loss", "val_mae", "best_val_mae",
               "batch_size", "learning_rate", "epoch", "end_epoch", "target_mae",
               "target_mae_deviation", "patience", "params", "config"]  # 'config' 컬럼 추가
    ...
    new_row = {'num.': num,
               ...
               'params': np.nan,
               'config': np.nan}  # 'config' 컬럼에 NaN으로 초기화
    ...
    df.loc[df['num.'] == num, 'config'] = str(config)
