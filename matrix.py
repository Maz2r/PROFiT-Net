...
f=open('Polarizability.table')
pol=[]
ohv_pol=[]
for i in range(102):
    ohv_zero=np.zeros(12)
    pol.append(float(f.readline()[:-1]))
    tmp=int(pol[-1]//5)
    ohv_zero[tmp]=1
    ohv_pol.append(ohv_zero)
f.close()

with open('cgcnn_atom_feature.json') as f:
    cgcnn_atom_feature=json.load(f)
...
class OrbitalFieldMatrix(BaseFeaturizer):
    ...
    def __init__(self, period_tag=False, flatten=True):
        ...
        for Z in range(1, 100):
            ...
            Z_str=str(Z)
            my_ohvs[Z] = np.append(my_ohvs[Z],cgcnn_atom_feature[Z_str])
            my_ohvs[Z] = np.append(my_ohvs[Z],ohv_pol[Z])
    ...
    def get_single_ofm(self, site, site_dict):
        ...
        """
        ohvs = self.ohvs
        atom_ofm = np.matrix(np.zeros((self.size+104, self.size+104)))
    ...
    def feature_labels(self):
        if self.flatten:
            ...
            for i in range(92):
                cgcnn_labels = ["cgcnn_" +str(i)]
                labelset_1D += cgcnn_labels

            for i in range(12):
                pol_labels = ["pol_" +str(i)]
                labelset_1D += pol_labels
            labelset_2D = []
