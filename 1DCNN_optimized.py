...
wandb.init(
    ...
    config={
        ...
        'learning_rate': 0.001,
        'epochs': 500,
        'target_mae': 0.65,               # 조기 종료를 위한 목표 MAE
        'target_mae_deviation': 0.03,    # 조기 종료를 위한 목표 MAE 편차
...
class MyModel(nn.Module):
    def __init__(self, sequence_length):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=22, kernel_size=12),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=5),
            nn.Dropout(0.022508308060965175),
            nn.Conv1d(in_channels=22, out_channels=19, kernel_size=9),
            nn.ReLU(),
            nn.Dropout(0.03811144266792502),
            nn.AvgPool1d(kernel_size=6),
            nn.Conv1d(in_channels=19, out_channels=15, kernel_size=10),
            nn.ReLU(),
            nn.Dropout(0.00833829926184717),
            nn.Conv1d(in_channels=15, out_channels=12, kernel_size=6),
            nn.ReLU(),
            nn.Dropout(0.0705106847529427),
            nn.Conv1d(in_channels=12, out_channels=12, kernel_size=1),
            nn.ReLU(),
            nn.Dropout(0.05538482581808256),
        )
...
# 추가 변수: 학습률 감소 여부 및 추가 에포크 카운터
lr_reduced = False
additional_epochs = 10  # 목표 조기 종료 시 추가로 학습할 에포크 수
    ...
    if len(val_mae_history) >= patience:
        ...
        if val_mae <= target_mae and mae_deviation <= target_mae_deviation:
            if not lr_reduced:
                ...
                print('학습률을 1/10배로 감소시킵니다.')
                # 학습률 감소
                for param_group in optimizer.param_groups:
                    param_group['lr'] *= 0.1
                print(f'새로운 learning rate: {optimizer.param_groups[0]["lr"]}')
                lr_reduced = True
                additional_epochs = 10  # 추가로 학습할 에포크 수 설정
            elif additional_epochs > 0:
                print(f'추가로 {additional_epochs} 에포크를 더 학습합니다.')
                additional_epochs -= 1
                if additional_epochs == 0:
                    print('추가 에포크 학습을 완료하여 학습을 중지합니다.')
